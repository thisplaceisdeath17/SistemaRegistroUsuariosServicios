...
public class ListadoCarreraArea extends JInternalFrame {
	...
	private String[] encabezado = {"Clave", "Nombre Carrera/Área", "Nombre Jéfe/Director", "Tipo"};
	private CarreraArea ca = new CarreraArea();
	private Object[][] datosCA = null;
	private DefaultTableModel modelo = null;
	private int cantidad;

	public ListadoCarreraArea(String[] usuario) {
		initComponents();
	...
	private void initComponents() {
		//Propiedades del lbl
		cantidad = ca.carreraAreaContar("Todos");
	...
	private void llenarTabla() {
		modelo = new DefaultTableModel(datosCA, encabezado) {
			@Override
	...
	private void cboFiltroActionPerformed(ActionEvent evt) {
		String filtro = null;
		if (cboFiltro.getSelectedItem().equals("Carrera")) {
			filtro = cboFiltro.getSelectedItem().toString();
			datosCA = ca.carreraAreaConsultar(filtro);
			cantidad = ca.carreraAreaContar(filtro);
			lblNumero.setText("# de Registros: " + cantidad);
			this.llenarTabla();
	...
	private void txtBusquedaKeyReleased(KeyEvent evt) {
		...
		if (cboFiltro.getSelectedItem().equals("Seleccione una opción")) {
			JOptionPane.showInternalMessageDialog(ListadoCarreraArea.this, "Debe seleccionar una opción de la lista desplagable antes de realizar una busqueda.", "Seleccion de opción. - SiRiUS", JOptionPane.WARNING_MESSAGE, warning);
			...
			if (busqueda.isEmpty()) {
				...
				datosCA = ca.carreraAreaBuscar(busqueda, filtro);
			}
			this.llenarTabla();
		}
	}
	...
	private void tableMouseClicked(MouseEvent evt) {
		if (usuario[2].equals("Administrador")) {
			JOptionPane.showInternalMessageDialog(ListadoCarreraArea.this, "Debes ser Super Administrador para seleccionar un registro.", "Selección denegada. - SiRiUS.", JOptionPane.WARNING_MESSAGE, warning);
		} else {
			if (evt.getClickCount() == 2) {
				String clave = table.getValueAt(table.getSelectedRow(), 0).toString();
				abrirSeleccionCA(clave);
				datosCA = ca.carreraAreaConsultar("Todos");
				this.dispose();
				this.llenarTabla();
			}
	...
	private String abrirSeleccionCA(String clave) {
		CarreraAreaSeleccionado cas = new CarreraAreaSeleccionado(usuario, clave);
		int x = (VentanaPrincipal.escritorio.getWidth() / 2) - cas.getWidth() / 2;
		int y = (VentanaPrincipal.escritorio.getHeight() / 2) - cas.getHeight() / 2;
		if (cas.isShowing()) {
			cas.setLocation(x, y);
		} else {
			VentanaPrincipal.escritorio.add(cas);
			cas.setLocation(x, y);
			cas.show();
		}
		return clave;
